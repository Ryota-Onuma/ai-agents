...
1. **プロジェクト文書の参照**
   ...
   - `CLAUDE.md` とそこから参照されるドキュメントを読み込み、対象システムの前提知識を得る
...
7. **ファイルの作成**
   ...
   - `docs/spec/{要件名}-acceptance-criteria.md`: 受け入れ基準とテスト項目（テスト項目は機能要件を直接検証する最小限のものに留める）
...
- {確認すべき副作用}
  ...
  **テストケース（機能要件を直接検証する最小限のもののみ）**:
  - [ ] 基本系: {主要なケースの詳細}
  - [ ] 例外系（必要な場合のみ）: {重要な例外や境界条件}

### REQ-002: {要件名} の受け入れ基準

{同様の形式で記載}

  ## Edgeケーステスト基準（必要な場合のみ）
